-- Import data for categories and products
-- Run after Hibernate has created the schema (application startup with database.generation=update)
-- Usage: psql -U postgres -d store -f import-data.sql

-- Categories (must run before products due to FK)
...
-- Reset sequence for categories (so next auto-insert gets id 4+)
...
-- Reset sequence for products (so next auto-insert gets id 302+)
